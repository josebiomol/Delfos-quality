          ...
          <tbody>
            ${currentAppointments.map((apt, idx) => `
              <tr style="border-bottom: 1px solid var(--line); background: ${idx % 2 === 0 ? 'var(--panel)' : 'var(--surface-alt)'};">
                ...
                <td style="padding: 1rem; font-size:var(--text-base);">${apt.horario ? Formatters.formatTime(apt.horario) : '—'}</td>
  ...
  exportToPDF(state) {
    ...
    const loadScript = (src) => {
      return new Promise((resolve, reject) => {
        ...
        if (existing) { existing.addEventListener('load', resolve); existing.addEventListener('error', () => reject(new Error('Falha ao carregar ' + src))); return; }
    ...
    // ✅ Carrega jsPDF PRIMEIRO e só então autoTable (que depende de window.jspdf.jsPDF já existir).
    // Carregar os dois em paralelo causava corrida: se autoTable terminasse antes do jsPDF,
    // ele falhava silenciosamente e pdf.autoTable() não existia (gerando erro falso).
    loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js')
      ...
      const rows = filtered.map(apt => [
        ...
        apt.horario ? Formatters.formatTime(apt.horario) : '—',
        apt.paciente || '—',
