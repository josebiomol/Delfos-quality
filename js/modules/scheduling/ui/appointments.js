...
export const AppointmentsUI = {
        ...
        <div style="display:flex;flex-direction:column;gap:1.5rem">
          <div style="display:flex;flex-direction:column;gap:1rem">
            ...
            <div style="display: flex; justify-content: flex-start; align-items: flex-end; gap: 12px; flex-wrap: wrap;">
              <button class="new-apt-btn icon-btn icon-btn-add" title="Novo agendamento" style="width:36px;height:36px;font-size:18px;flex-shrink:0">+</button>
  ...
  exportToPDF(state) {
    ...
    const needsJsPdf = !window.jspdf;
    const needsAutoTable = !(window.jspdf && window.jspdf.jsPDF && window.jspdf.jsPDF.API && window.jspdf.jsPDF.API.autoTable);

    const scriptsToLoad = [];
    if (needsJsPdf) scriptsToLoad.push(loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js'));
    if (needsAutoTable) scriptsToLoad.push(loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.8.2/jspdf.plugin.autotable.min.js'));

    Promise.all(scriptsToLoad).then(() => {
      if (!window.jspdf || !window.jspdf.jsPDF) {
        throw new Error('jsPDF não carregou corretamente (window.jspdf indisponível)');
      }
