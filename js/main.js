...
async function init() {
  ...
  const isFreshLogin = sessionStorage.getItem('freshLogin') === '1';
  ...
  } else if (!isFreshLogin && savedUnit && state.units.some(u => u.unidade_id === savedUnit.unidade_id)) {
    // Recarregar a página (F5) não deve pedir a unidade de novo — só no login.
    state.unit = savedUnit;
    await loadAndRenderDashboard();
  } else if (state.units.length > 0) {
    // No login, sempre mostra a tela de seleção de unidade —
    // é ela quem aplica o filtro de dados da unidade.
    renderChangeUnit();
  }
}
    ...
    onLogin: async (data) => {
      ...
      try {
        ...
        if (response.success) {
          ...
          sessionStorage.setItem('freshLogin', '1'); // força tela de seleção de unidade só no login, não em refresh
          try {
            await init();
          } finally {
            hideLoadingOverlay();
...
function renderChangeUnit() {
  ...
  const unitRows = units
    .map((u, idx) => `
      <button class="unit-card ${idx % 2 === 1 ? 'odd' : ''}" data-id="${u.unidade_id}" style="display:flex;align-items:center;gap:14px;padding:14px 16px;border-bottom:1px solid var(--line);background:${idx % 2 === 1 ? 'var(--surface-alt)' : 'transparent'};border-left:none;border-right:none;border-top:none;cursor:pointer;width:100%;text-align:left;color:inherit">
        <div style="flex-shrink:0;width:34px;height:34px;border-radius:8px;border:1px solid var(--line);background:var(--panel);display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:var(--text-base)">${renderIcon('HOSPITAL')}</div>
        <div style="flex:1;min-width:0">
          <h3 style="margin:0;color:var(--text);font-size:var(--text-base);font-weight:var(--font-semibold)">${u.nome_unidade}</h3>
          <p style="margin:2px 0 0 0;color:var(--muted);font-size:var(--text-xs);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${u.endereco || 'Endereço não definido'}</p>
        </div>
  ...
  const modalHTML = `
    <div style="display:flex;flex-direction:column;min-height:100vh;background:var(--bg);padding:1.5rem">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;width:100%;max-width:600px;margin:0 auto 1.5rem">
        <div>
          <h1 style="margin:0;color:var(--text);font-size:var(--text-2xl);font-weight:var(--font-bold)">Selecione a unidade</h1>
          <p style="margin:0.25rem 0 0 0;color:var(--muted);font-size:var(--text-sm)">Escolha onde deseja trabalhar agora</p>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
          ${window.security?.permissionMiddleware?.isActionAllowed('add_unidade')
            ? `<button id="newUnitBtn" class="icon-btn icon-btn-add" title="Nova unidade" style="width:36px;height:36px;font-size:18px">+</button>`
            : ''}
          <button id="logoutBtn" class="icon-btn" title="Sair" style="color:var(--muted)">${renderIcon('LOGOUT')}</button>
        </div>
      </div>

      <div style="width:100%;max-width:600px;margin:0 auto;border:1px solid var(--line);border-radius:8px;overflow:hidden;background:var(--panel)">
        ${unitRows}
      </div>
    </div>
  `;
...
function renderProfileEdit() {
  ...
  ProfileUI.bind(state, {
    onBack: () => renderShell(state.user?.pagina_inicial || lastPageBeforeProfile || 'dashboard'),
    onChangeUnit: () => renderChangeUnit(),
    onLogout: () => {
      sessionStorage.removeItem('activePage');
      clearSecurityOnLogout();
    ...
    onSave: async (payload) => {
      try {
        ...
        if (response.success) {
          // Atualizar state em memória (todos os campos editáveis, não só nome/foto)
          state.user.nome = payload.nome;
          state.user.pagina_inicial = payload.pagina_inicial;
          state.user.tema_preferencia = payload.tema_preferencia;
          ...
          try {
            ...
            if (stored) {
              ...
              stored.pagina_inicial = payload.pagina_inicial;
              stored.tema_preferencia = payload.tema_preferencia;
          ...
          // Aplicar tema imediatamente (sem precisar deslogar)
          initTheme(payload.tema_preferencia);

          toast.show('✅ Perfil atualizado!', 'success');
          // Permanece na tela de Preferências (não navega para Configurações)
          renderProfileEdit();
        }
        return response;
      } catch (error) {
