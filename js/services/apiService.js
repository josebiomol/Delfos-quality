...
export async function api(action, payload = {}) {
  ...
  // Só lança exceção pra erros INESPERADOS do servidor (ex: exceção não tratada
  // no handle(), que retorna { error: '...' } sem 'success').
  // Falhas de negócio esperadas (senha errada, fora do horário, permissão
  // negada, login duplicado etc) vêm como { success:false, error:'...' } e
  // devem ser tratadas pelo chamador (if/else em response.success), não aqui —
  // senão a mensagem específica nunca chega na tela.
  if (data.error && data.success === undefined) throw new Error(data.error);
  return data;
}

// ========== SESSÃO ==========
export function saveSession(data) {
  localStorage.setItem('session', JSON.stringify({ user: data.user, units: data.units, time: Date.now() }));
}
...
async function mock(action, payload){
  ...
  if(action==='login') return { success: true, data: { user: db.user, units: db.units } };
