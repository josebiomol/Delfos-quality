/**
 * rateLimiter.js - ADAPTADO PARA AGENDA
 * Rate limiter para proteção contra brute force
 */

class RateLimiter {
  constructor(config = {}) {
    this.maxAttempts = config.maxAttempts || 5; // 5 tentativas
    this.windowMs = config.windowMs || 15 * 60 * 1000; // 15 minutos
    this.lockoutDurationMs = config.lockoutDurationMs || 30 * 60 * 1000; // 30 minutos
    this.attempts = new Map(); // { "email:login" → [timestamp1, timestamp2, ...] }
    this.lockouts = new Map(); // { "email:login" → lockoutTime }
  ...
  isBlocked(identifier, action = 'default') {
    ...
    if (now > lockoutTime) {
      ...
      console.log(`✅ Desbloqueio automático: ${lockoutKey}`);
      return false;
    }
  ...
  check(identifier, action = 'default') {
    ...
    if (this.isBlocked(identifier, action)) {
      ...
      console.warn(`🚫 Acesso bloqueado (rate limit): ${identifier}:${action}`);
    ...
    if (result.attempts > this.maxAttempts) {
      ...
      console.warn(`🚫 BLOQUEADO POR RATE LIMIT: ${lockoutKey}`);
    ...
    if (warning) {
      console.warn(`⚠️ ATENÇÃO: ${result.remainingAttempts} tentativa(s) restante(s) - ${identifier}:${action}`);
    }
    ...
    return {
      ...
      warning: warning,
      warningMessage: warning ? `⚠️ Atenção: ${result.remainingAttempts} tentativa(s) restante(s)` : null
    };
  }
  ...
  unlock(identifier, action = 'default') {
    const lockoutKey = this.getKey(identifier, action);
    this.lockouts.delete(lockoutKey);
    console.log(`🔓 Desbloqueio manual: ${lockoutKey}`);
  }
  ...
  cleanup() {
    ...
    if (cleaned > 0) {
      console.log(`🧹 Limpeza de rate limiter: ${cleaned} entradas removidas`);
    }
  }
